...
export default function PerformancePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editTarget ? { ...form, rowIndex: editTarget.rowIndex } : form;
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch('/api/performance', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rowIndex: deleteTarget }) });
      const data = await res.json();
                    ...
                    <tr key={r.rowIndex}>
                      ...
                      <td>
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => setDeleteTarget(r.rowIndex)} className="text-[#94a3b8] hover:text-red-500 p-1" title="刪除">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
